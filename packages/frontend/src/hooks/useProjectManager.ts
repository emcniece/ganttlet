...
import type { Task, AppSettings, Project, ProjectData, ProjectVisibility } from '@ganttlet/shared'
...
export function useProjectManager() {
  ...
  // Server-side project tracking (set when project is saved to server)
  const [serverProjectId, setServerProjectId] = useLocalStorage<string | null>(
    `gantt-server-project-${activeProjectId}`,
    null
  )
  ...
  const deleteProject = useCallback((id: string) => {
    setProjects((prev) => {
      ...
      try { localStorage.removeItem(`gantt-server-project-${id}`) } catch { /* ignore */ }
